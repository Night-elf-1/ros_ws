...
public:
  DynamicTFBroadcaster() : Node("dynamic_tf_broadcaster")
  {
    ...
    timer_ = create_wall_timer(10ms, std::bind(&DynamicTFBroadcaster::publishTransform, this));         // 定时器自动调用函数
  }
...
private:
    std::shared_ptr<tf2_ros::TransformBroadcaster> tf_broadcaster_;               // 创建TF变换广播者变量的共享指针
