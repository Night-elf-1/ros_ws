...
class TFListener : public rclcpp::Node {
 public:
  TFListener() : Node("tf_listener") {
    buffer_ = std::make_unique<tf2_ros::Buffer>(this->get_clock());                     // 传入当前时间戳
 ...
 private:
  std::unique_ptr<tf2_ros::Buffer> buffer_;                     // 声明buffer类的智能指针，用于存储订阅到的TF数据
  std::shared_ptr<tf2_ros::TransformListener> listener_;        // 声明监听者
